...
.. csv-table:: ELMFIRE Qualitiative Forecast Skill for 214 Forecasts Issued During 2022.
   :align: center
...
``$ELMFIRE_BASE_DIR/cloudfire`` is in the curren user's ``$PATH``, the 
...
setting the flag ``--active=False`` and then spefying the desired year 
...
built from satellit hot spot data (left panel), GIS operations such as a 
...
panel by 500 m and then buffereing that result by -500 m.
