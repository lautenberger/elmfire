...
ELMFIRE_POST=/usr/local/bin/elmfire_post_$ELMFIRE_VER
...
mpirun --mca btl tcp,self --allow-run-as-root --map-by core --bind-to core --oversubscribe -np $NP -x OMP_NUM_THREADS $ELMFIRE_POST elmfire_post.data #2> /dev/null
