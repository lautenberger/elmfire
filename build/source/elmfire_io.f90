...
! 32: TIMES_BURNED
! 33: SURFACE_FIRE_AREA
! 34: CROWN_FIRE_AREA
! 35: SURFACE_FIRE_VOLUME
! 36: AFFECTED_POPULATION
! 37: AFFECTED_REAL_ESTATE_VALUE
! 38: AFFECTED_LAND_VALUE
   I = 0
   DO J = 32, 38
      PARALLEL_IO_RANK(J) = I
      I = I + 1
...
IF (USE_HAMADA) THEN
   IF (USE_CONSTANT_HAMADA_PARAMETERS) THEN
      IF (IRANK_WORLD .EQ. PARALLEL_IO_RANK(28)) HAMADA_A%R4 (:,:,:) = HAMADA_A_CONSTANT
      IF (IRANK_WORLD .EQ. PARALLEL_IO_RANK(29)) HAMADA_D%R4 (:,:,:) = HAMADA_D_CONSTANT
      IF (IRANK_WORLD .EQ. PARALLEL_IO_RANK(30)) HAMADA_FB%R4(:,:,:) = HAMADA_FB_CONSTANT
   ELSE
      IF (IRANK_WORLD .EQ. PARALLEL_IO_RANK(28)) CALL READ_BSQ_RASTER (HAMADA_A,  FUELS_AND_TOPOGRAPHY_DIRECTORY, HAMADA_A_FILENAME )
      IF (IRANK_WORLD .EQ. PARALLEL_IO_RANK(29)) CALL READ_BSQ_RASTER (HAMADA_D,  FUELS_AND_TOPOGRAPHY_DIRECTORY, HAMADA_D_FILENAME )
      IF (IRANK_WORLD .EQ. PARALLEL_IO_RANK(30)) CALL READ_BSQ_RASTER (HAMADA_FB, FUELS_AND_TOPOGRAPHY_DIRECTORY, HAMADA_FB_FILENAME)
   ENDIF
ENDIF

IF (USE_PYROMES .AND. IRANK_WORLD .EQ. PARALLEL_IO_RANK(31)) CALL READ_BSQ_RASTER (PYROMES, FUELS_AND_TOPOGRAPHY_DIRECTORY, PYROMES_FILENAME)
...
IF (NPROC .GT. 1) THEN
   ...
   IF (USE_HAMADA) THEN
      CALL MPI_BCAST_RASTER_HEADER(HAMADA_A , PARALLEL_IO_RANK(28), .FALSE.)
      CALL MPI_BCAST_RASTER_HEADER(HAMADA_D , PARALLEL_IO_RANK(29), .FALSE.)
      CALL MPI_BCAST_RASTER_HEADER(HAMADA_FB, PARALLEL_IO_RANK(30), .FALSE.)
   ENDIF
   IF (USE_PYROMES) CALL MPI_BCAST_RASTER_HEADER(PYROMES, PARALLEL_IO_RANK(31), .FALSE.)
ENDIF
...
IF (USE_HAMADA) THEN
   ...
   ELSE
      IF (IRANK_WORLD .EQ. PARALLEL_IO_RANK(28)) THEN
         FN = TRIM(FUELS_AND_TOPOGRAPHY_DIRECTORY) // TRIM(HAMADA_A_FILENAME)
         CALL READ_BSQ_RASTER_EXISTING_TILED (HAMADA_A,FN)
      ENDIF
      IF (IRANK_WORLD .EQ. PARALLEL_IO_RANK(29)) THEN
         FN = TRIM(FUELS_AND_TOPOGRAPHY_DIRECTORY) // TRIM(HAMADA_D_FILENAME)
         CALL READ_BSQ_RASTER_EXISTING_TILED (HAMADA_D,FN)
      ENDIF
      IF (IRANK_WORLD .EQ. PARALLEL_IO_RANK(30)) THEN
         FN = TRIM(FUELS_AND_TOPOGRAPHY_DIRECTORY) // TRIM(HAMADA_FB_FILENAME)
         CALL READ_BSQ_RASTER_EXISTING_TILED (HAMADA_FB,FN)
      ENDIF
   ENDIF
ENDIF

IF (USE_PYROMES .AND. IRANK_WORLD .EQ. PARALLEL_IO_RANK(31)) THEN
...
IF (NPROC .GT. 1) THEN
   ...
   IF (USE_LAND_VALUE        ) CALL MPI_BCAST_RASTER_HEADER(LAND_VALUE         , PARALLEL_IO_RANK(25), .FALSE.)
   IF (USE_SDI               ) CALL MPI_BCAST_RASTER_HEADER(SDI                , PARALLEL_IO_RANK(27), .FALSE.)
   IF (USE_HAMADA .AND. (.NOT. USE_CONSTANT_HAMADA_PARAMETERS)) THEN
      CALL MPI_BCAST_RASTER_HEADER(HAMADA_A , PARALLEL_IO_RANK(28), .FALSE.)
      CALL MPI_BCAST_RASTER_HEADER(HAMADA_D , PARALLEL_IO_RANK(29), .FALSE.)
      CALL MPI_BCAST_RASTER_HEADER(HAMADA_FB, PARALLEL_IO_RANK(30), .FALSE.)
   ENDIF
...
IF (DUMP_TRANSIENT_ACREAGE) THEN
   FN = TRIM(OUTPUTS_DIRECTORY) // 'acres_' // SIX // '.csv'
...
IF (DUMP_TIME_OF_ARRIVAL .AND. IDUMPCOUNT .EQ. NDUMPS) THEN
   ...
   DO I = 1, LIST_BURNED%NUM_NODES
      ...
      C => C%NEXT
   ENDDO
   FN = 'time_of_arrival_' // SIX // '_' // CTSEC
   CALL WRITE_BIL_RASTER(RDUMPME,OUTPUTS_DIRECTORY,FN,CONVERT_TO_GEOTIFF,.TRUE.)
...
IF (DUMP_ISOCHRONE_SHAPEFILES) THEN
   FN = 'isochrones_' // SIX // '_' // CTSEC

   IF (.NOT. DUMPEDYET) THEN
      SHELLSTR = TRIM(DELETECOMMAND) // " " // TRIM(SCRATCH) // "isochrones_" // SIX // "*.shp"; WRITE(*,100) TRIM(SHELLSTR)
      ISTAT = SYSTEM(TRIM(SHELLSTR))
      SHELLSTR = TRIM(DELETECOMMAND) // " " // TRIM(SCRATCH) // "isochrones_" // SIX // "*.shx"; WRITE(*,100) TRIM(SHELLSTR)
      ISTAT = SYSTEM(TRIM(SHELLSTR))
      SHELLSTR = TRIM(DELETECOMMAND) // " " // TRIM(SCRATCH) // "isochrones_" // SIX // "*.dbf"; WRITE(*,100) TRIM(SHELLSTR)
      ISTAT = SYSTEM(TRIM(SHELLSTR))
      SHELLSTR = TRIM(DELETECOMMAND) // " " // TRIM(OUTPUTS_DIRECTORY) // SIX // "*" ; WRITE(*,100) TRIM(SHELLSTR)
      ISTAT = SYSTEM(TRIM(SHELLSTR))       
   ENDIF
...
! Convert newly dumped phi raster to shapefile in scratch space. 
   ...
   IF (.NOT. DUMPEDYET) THEN
      SHELLSTR = TRIM(OGR2OGR) // ' -skipfailures -nlt MULTILINESTRING -a_srs "' // TRIM(A_SRS) // '" ' // TRIM(OUTPUTS_DIRECTORY) // SIX // ".shp "  // TRIM(SCRATCH) // TRIM(FN) // ".shp" // &
     ' -dialect sqlite -sql "SELECT ST_Union(geometry) AS geometry FROM ' // TRIM(FN) // '"'

      WRITE(*,100) TRIM(SHELLSTR); ISTAT = SYSTEM(TRIM(SHELLSTR)) 
   ELSE
      SHELLSTR = TRIM(OGR2OGR) // ' -skipfailures -nlt MULTILINESTRING -update -append -a_srs "' // TRIM(A_SRS) // '" ' // TRIM(OUTPUTS_DIRECTORY) // SIX // ".shp "  // &
   ...
   SHELLSTR = TRIM(DELETECOMMAND) // " " // TRIM(SCRATCH) // 'isochrones_' // SIX // '*'; WRITE(*,100) TRIM(SHELLSTR)
   ISTAT = SYSTEM(TRIM(SHELLSTR)) 
   DUMPEDYET = .TRUE.       
...
IF (CONVERT_TO_GEOTIFF) THEN
      IF (DUMP_SURFACE_FIRE          .AND. CONVERT_TO_GEOTIFF) CALL MERGE_RASTERS(SIX, FN_SURFACE_FIRE_TIF       , FN_SURFACE_FIRE_HDR       , "surface_fire_      ")
      IF (DUMP_CROWN_FIRE            .AND. CONVERT_TO_GEOTIFF) CALL MERGE_RASTERS(SIX, FN_CROWN_FIRE_TIF         , FN_CROWN_FIRE_HDR         , "crown_fire_        ")
      IF (DUMP_SPREAD_RATE           .AND. CONVERT_TO_GEOTIFF) CALL MERGE_RASTERS(SIX, FN_SPREAD_RATE_TIF        , FN_SPREAD_RATE_HDR        , "spread_rate_       ")
      IF (DUMP_HPUA                  .AND. CONVERT_TO_GEOTIFF) CALL MERGE_RASTERS(SIX, FN_HPUA_TIF               , FN_HPUA_HDR               , "hpua_              ")
      IF (DUMP_FLIN                  .AND. CONVERT_TO_GEOTIFF) CALL MERGE_RASTERS(SIX, FN_FLIN_TIF               , FN_FLIN_HDR               , "flin_              ")
      IF (DUMP_FLAME_LENGTH          .AND. CONVERT_TO_GEOTIFF) CALL MERGE_RASTERS(SIX, FN_FLAME_LENGTH_TIF       , FN_FLAME_LENGTH_HDR       , "flame_length_      ")
      IF (DUMP_REACTION_INTENSITY    .AND. CONVERT_TO_GEOTIFF) CALL MERGE_RASTERS(SIX, FN_REACTION_INTENSITY_TIF , FN_REACTION_INTENSITY_HDR , "reaction_intensity_")
      IF (DUMP_WS20                  .AND. CONVERT_TO_GEOTIFF) CALL MERGE_RASTERS(SIX, FN_WS20_TIF               , FN_WS20_HDR               , "ws20_              ")
      IF (DUMP_WD20                  .AND. CONVERT_TO_GEOTIFF) CALL MERGE_RASTERS(SIX, FN_WD20_TIF               , FN_WD20_HDR               , "wd20_              ")
      IF (DUMP_VELOCITY              .AND. CONVERT_TO_GEOTIFF) CALL MERGE_RASTERS(SIX, FN_VELOCITY_TIF           , FN_VELOCITY_HDR           , "velocity_          ")
      IF (DUMP_SPOTTING_IGNITION_TIME.AND. CONVERT_TO_GEOTIFF) CALL MERGE_RASTERS(SIX, FN_SPOTTING_IGN_TIME_TIF  , FN_SPOTTING_IGN_TIME_HDR  , "spot_ign_time_     ")
      IF (DUMP_TIME_OF_ARRIVAL       .AND. CONVERT_TO_GEOTIFF) CALL MERGE_RASTERS(SIX, FN_TIME_OF_ARRIVAL_TIF    , FN_TIME_OF_ARRIVAL_HDR    , "toa_               ")
      IF (DUMP_TAGGED                .AND. CONVERT_TO_GEOTIFF) CALL MERGE_RASTERS(SIX, FN_TAGGED_TIF             , FN_TAGGED_HDR             , "tagged_            ")
      IF (DUMP_PHI                   .AND. CONVERT_TO_GEOTIFF) CALL MERGE_RASTERS(SIX, FN_PHI_TIF                , FN_PHI_HDR                , "phi_               ")
      IF (.NOT. ACCUMULATE_EMBER_FLUX) THEN
         IF (DUMP_EMBER_FLUX            .AND. CONVERT_TO_GEOTIFF) CALL MERGE_RASTERS(SIX, FN_EMBER_FLUX_TIF         , FN_EMBER_FLUX_HDR         , "ember_flux_        ")
      ENDIF
ELSE
      IF (DUMP_SURFACE_FIRE          .AND. CONVERT_TO_GEOTIFF) CALL MERGE_RASTERS(SIX, FN_SURFACE_FIRE_BIL       , FN_SURFACE_FIRE_HDR       , "surface_fire_      ")
      IF (DUMP_CROWN_FIRE            .AND. CONVERT_TO_GEOTIFF) CALL MERGE_RASTERS(SIX, FN_CROWN_FIRE_BIL         , FN_CROWN_FIRE_HDR         , "crown_fire_        ")
      IF (DUMP_SPREAD_RATE           .AND. CONVERT_TO_GEOTIFF) CALL MERGE_RASTERS(SIX, FN_SPREAD_RATE_BIL        , FN_SPREAD_RATE_HDR        , "spread_rate_       ")
      IF (DUMP_HPUA                  .AND. CONVERT_TO_GEOTIFF) CALL MERGE_RASTERS(SIX, FN_HPUA_BIL               , FN_HPUA_HDR               , "hpua_              ")
      IF (DUMP_FLIN                  .AND. CONVERT_TO_GEOTIFF) CALL MERGE_RASTERS(SIX, FN_FLIN_BIL               , FN_FLIN_HDR               , "flin_              ")
      IF (DUMP_FLAME_LENGTH          .AND. CONVERT_TO_GEOTIFF) CALL MERGE_RASTERS(SIX, FN_FLAME_LENGTH_BIL       , FN_FLAME_LENGTH_HDR       , "flame_length_      ")
      IF (DUMP_REACTION_INTENSITY    .AND. CONVERT_TO_GEOTIFF) CALL MERGE_RASTERS(SIX, FN_REACTION_INTENSITY_BIL , FN_REACTION_INTENSITY_HDR , "reaction_intensity_")
      IF (DUMP_WS20                  .AND. CONVERT_TO_GEOTIFF) CALL MERGE_RASTERS(SIX, FN_WS20_BIL               , FN_WS20_HDR               , "ws20_              ")
      IF (DUMP_WD20                  .AND. CONVERT_TO_GEOTIFF) CALL MERGE_RASTERS(SIX, FN_WD20_BIL               , FN_WD20_HDR               , "wd20_              ")
      IF (DUMP_VELOCITY              .AND. CONVERT_TO_GEOTIFF) CALL MERGE_RASTERS(SIX, FN_VELOCITY_BIL           , FN_VELOCITY_HDR           , "velocity_          ")
      IF (DUMP_SPOTTING_IGNITION_TIME.AND. CONVERT_TO_GEOTIFF) CALL MERGE_RASTERS(SIX, FN_SPOTTING_IGN_TIME_BIL  , FN_SPOTTING_IGN_TIME_HDR  , "spot_ign_time_     ")
      IF (DUMP_TIME_OF_ARRIVAL       .AND. CONVERT_TO_GEOTIFF) CALL MERGE_RASTERS(SIX, FN_TIME_OF_ARRIVAL_BIL    , FN_TIME_OF_ARRIVAL_HDR    , "toa_               ")
      IF (DUMP_TAGGED                .AND. CONVERT_TO_GEOTIFF) CALL MERGE_RASTERS(SIX, FN_TAGGED_BIL             , FN_TAGGED_HDR             , "tagged_            ")
      IF (DUMP_PHI                   .AND. CONVERT_TO_GEOTIFF) CALL MERGE_RASTERS(SIX, FN_PHI_BIL                , FN_PHI_HDR                , "phi_               ")
      IF (.NOT. ACCUMULATE_EMBER_FLUX) THEN
         IF (DUMP_EMBER_FLUX            .AND. CONVERT_TO_GEOTIFF) CALL MERGE_RASTERS(SIX, FN_EMBER_FLUX_BIL         , FN_EMBER_FLUX_HDR         , "ember_flux_        ")
      ENDIF
ENDIF
...
! *****************************************************************************
SUBROUTINE MERGE_RASTERS(SIX,FN_LIST,FN_HDRS,STUB)
! *****************************************************************************
...
FNOUT=TRIM(OUTPUTS_DIRECTORY) // TRIM(STUB) // SIX // '.tif'

IF (TRIM(SCRATCH) .EQ. 'null' .OR. LEN_TRIM(SCRATCH) .LE. 1) THEN
   FNTMP=TRIM(OUTPUTS_DIRECTORY) // SIX // "_intermediate.tif " 
ELSE
   FNTMP=TRIM(SCRATCH) // SIX // "_intermediate.tif " 
ENDIF
...
IF (CALCULATE_TIMES_BURNED .AND. IRANK_WORLD .EQ. PARALLEL_IO_RANK(32)) THEN
   FN='times_burned'
...
IF (DUMP_SURFACE_FIRE_AREA .AND. IRANK_WORLD .EQ. PARALLEL_IO_RANK(33)) THEN
...
IF (DUMP_CROWN_FIRE_AREA .AND. IRANK_WORLD .EQ. PARALLEL_IO_RANK(34)) THEN
...
IF (DUMP_FIRE_VOLUME .AND. IRANK_WORLD .EQ. PARALLEL_IO_RANK(35)) THEN
   PREFIX = 'fire_volume'
...
IF (DUMP_AFFECTED_POPULATION .AND. IRANK_WORLD .EQ. PARALLEL_IO_RANK(36)) THEN
...
IF (DUMP_AFFECTED_REAL_ESTATE_VALUE .AND. IRANK_WORLD .EQ. PARALLEL_IO_RANK(37)) THEN
...
IF (DUMP_AFFECTED_LAND_VALUE .AND. IRANK_WORLD .EQ. PARALLEL_IO_RANK(38)) THEN
