...
! *****************************************************************************
   ...
   INTEGER, INTENT(OUT) :: SAMPLES, LINES, BANDS, DATA_TYPE
   INTEGER, INTENT(OUT) :: HEADER_OFFSET, BYTE_ORDER
   REAL, INTENT(OUT) :: DATA_IGNORE_VALUE
   CHARACTER(LEN=3), INTENT(OUT) :: INTERLEAVE
   CHARACTER(LEN=1024), INTENT(OUT) :: COORDINATE_SYSTEM, DESCRIPTION
   CHARACTER(LEN=1024), INTENT(OUT) :: DEFAULT_BANDS, BAND_NAMES
   CHARACTER(LEN=64), INTENT(OUT) :: FILE_TYPE
   ! Map Info Parameters
   CHARACTER(64), INTENT(OUT) :: PROJECTION_NAME
   REAL, INTENT(OUT) :: X_PIXEL_REFERENCE
   REAL, INTENT(OUT) :: Y_PIXEL_REFERENCE
   REAL, INTENT(OUT) :: EASTING
   REAL, INTENT(OUT) :: NORTHING
   REAL, INTENT(OUT) :: X_PIXEL_SIZE
   REAL, INTENT(OUT) :: Y_PIXEL_SIZE

   CHARACTER(LEN=256), DIMENSION(7) :: MAP_INFO_PARAMS
   CHARACTER(LEN=1024) :: LINE, CURRENT_KEY, CURRENT_VALUE
   INTEGER :: IUNIT, IOS
   LOGICAL :: READING_MULTILINE_VALUE, FOUND_SAMPLES, FOUND_LINES, FOUND_BANDS
   LOGICAL :: FOUND_DATA_TYPE, FOUND_BYTE_ORDER, FOUND_DATA_IGNORE_VALUE
   LOGICAL :: FOUND_MAP_INFO, FOUND_COORDINATE_SYSTEM, FOUND_DESCRIPTION
   LOGICAL :: FOUND_FILE_TYPE, FOUND_DEFAULT_BANDS, FOUND_INTERLEAVE
   LOGICAL :: FOUND_HEADER_OFFSET, FOUND_BAND_NAMES
   ...
   FOUND_LINES = .FALSE.
   FOUND_BANDS = .FALSE.
   FOUND_HEADER_OFFSET = .FALSE.
   FOUND_FILE_TYPE = .FALSE.
   FOUND_DATA_TYPE = .FALSE.
   FOUND_INTERLEAVE = .FALSE.
   FOUND_BYTE_ORDER = .FALSE.
   FOUND_MAP_INFO = .FALSE.
   FOUND_COORDINATE_SYSTEM = .FALSE.
   FOUND_BAND_NAMES = .FALSE.
   FOUND_DATA_IGNORE_VALUE = .FALSE.
   FOUND_DEFAULT_BANDS = .FALSE.
   ...
   DO
      ...
      IF (READING_MULTILINE_VALUE) THEN
         ! CONTINUE READING UNTIL THE MULTILINE VALUE ENDS
         IF (INDEX(LINE, '}') > 0) THEN
         CURRENT_VALUE = TRIM(CURRENT_VALUE) // ' ' // TRIM(ADJUSTL(LINE))
         READING_MULTILINE_VALUE = .FALSE.
         ELSE
         CURRENT_VALUE = TRIM(CURRENT_VALUE) // ' ' // TRIM(ADJUSTL(LINE))
         CYCLE
         ENDIF
      ELSE
         CALL SPLIT_LINE(LINE, CURRENT_KEY, CURRENT_VALUE)
         IF (INDEX(CURRENT_VALUE, '{') > 0 .AND. INDEX(CURRENT_VALUE, '}') == 0) THEN
         READING_MULTILINE_VALUE = .TRUE.
         CYCLE
         ENDIF
      ENDIF

      ! MATCH KEY TO EXPECTED VARIABLES AND STORE VALUES
      SELECT CASE (TRIM(ADJUSTL(CURRENT_KEY)))
      CASE ('description')
         DESCRIPTION = TRIM(ADJUSTL(CURRENT_VALUE)); FOUND_DESCRIPTION = .TRUE.
      CASE ('samples')
         READ(CURRENT_VALUE, *) SAMPLES; FOUND_SAMPLES = .TRUE.
      CASE ('lines')
         READ(CURRENT_VALUE, *) LINES; FOUND_LINES = .TRUE.
      CASE ('bands')
         READ(CURRENT_VALUE, *) BANDS; FOUND_BANDS = .TRUE.
      CASE ('header offset')
         READ(CURRENT_VALUE, *) HEADER_OFFSET; FOUND_HEADER_OFFSET = .TRUE.
      CASE ('file type')
         FILE_TYPE = TRIM(ADJUSTL(CURRENT_VALUE)); FOUND_FILE_TYPE = .TRUE.
      CASE ('data type')
         READ(CURRENT_VALUE, *) DATA_TYPE; FOUND_DATA_TYPE = .TRUE.
      CASE ('interleave')
         INTERLEAVE = TRIM(ADJUSTL(CURRENT_VALUE)); FOUND_INTERLEAVE = .TRUE.
      CASE ('byte order')
         READ(CURRENT_VALUE, *) BYTE_ORDER; FOUND_BYTE_ORDER = .TRUE.
      CASE ('map info')
         CALL PARSE_MAP_INFO(TRIM(CURRENT_VALUE), MAP_INFO_PARAMS)
         FOUND_MAP_INFO = .TRUE.
         READ(MAP_INFO_PARAMS(1), *) PROJECTION_NAME
         READ(MAP_INFO_PARAMS(2), *) X_PIXEL_REFERENCE
         READ(MAP_INFO_PARAMS(3), *) Y_PIXEL_REFERENCE
         READ(MAP_INFO_PARAMS(4), *) EASTING
         READ(MAP_INFO_PARAMS(5), *) NORTHING
         READ(MAP_INFO_PARAMS(6), *) X_PIXEL_SIZE
         READ(MAP_INFO_PARAMS(7), *) Y_PIXEL_SIZE
      CASE ('coordinate system string')
         COORDINATE_SYSTEM = TRIM(ADJUSTL(CURRENT_VALUE)); FOUND_COORDINATE_SYSTEM = .TRUE.
      CASE ('band names')
         BAND_NAMES = TRIM(ADJUSTL(CURRENT_VALUE)); FOUND_BAND_NAMES = .TRUE.
      CASE ('data ignore value')
         READ(CURRENT_VALUE, *) DATA_IGNORE_VALUE; FOUND_DATA_IGNORE_VALUE = .TRUE.
      CASE ('default bands')
         DEFAULT_BANDS = TRIM(ADJUSTL(CURRENT_VALUE)); FOUND_DEFAULT_BANDS = .TRUE.
      END SELECT
   END DO
   ...
   IF (.NOT. FOUND_DESCRIPTION) DESCRIPTION = 'UNKNOWN'
   IF (.NOT. FOUND_SAMPLES) SAMPLES = -1
   IF (.NOT. FOUND_LINES) LINES = -1
   IF (.NOT. FOUND_BANDS) BANDS = 1
   IF (.NOT. FOUND_HEADER_OFFSET) HEADER_OFFSET = 0
   IF (.NOT. FOUND_FILE_TYPE) FILE_TYPE = 'UNKNOWN'
   IF (.NOT. FOUND_DATA_TYPE) DATA_TYPE = -1
   IF (.NOT. FOUND_INTERLEAVE) INTERLEAVE = 'UNKNOWN'
   IF (.NOT. FOUND_BYTE_ORDER) BYTE_ORDER = -1
   IF (.NOT. FOUND_MAP_INFO) THEN
      ...
      Y_PIXEL_SIZE = 0
   ENDIF
   IF (.NOT. FOUND_COORDINATE_SYSTEM) COORDINATE_SYSTEM = 'UNKNOWN'
   IF (.NOT. FOUND_BAND_NAMES) BAND_NAMES = 'UNKNOWN'
   IF (.NOT. FOUND_DATA_IGNORE_VALUE) DATA_IGNORE_VALUE = -9999.0
   IF (.NOT. FOUND_DEFAULT_BANDS) DEFAULT_BANDS = 'UNKNOWN'
...
! *****************************************************************************
SUBROUTINE SPLIT_LINE(LINE, KEY, VALUE)
! *****************************************************************************
   IMPLICIT NONE
...
! *****************************************************************************
SUBROUTINE PARSE_MAP_INFO(MAP_INFO_STR, MAP_INFO_PARAMS)
! *****************************************************************************
   IMPLICIT NONE
   CHARACTER(LEN=*), INTENT(IN) :: MAP_INFO_STR
   CHARACTER(LEN=256), DIMENSION(7), INTENT(OUT) :: MAP_INFO_PARAMS
   CHARACTER(LEN=256) :: TEMP_STR
   INTEGER :: I, N

   ! REMOVE BRACES AND SPLIT BY COMMAS
   TEMP_STR = MAP_INFO_STR
   TEMP_STR = TRIM(ADJUSTL(TEMP_STR))
   TEMP_STR = TEMP_STR(2:LEN(TEMP_STR)-1) ! REMOVE '{' AND '}'

   N = 1
   DO I = 1, 7
      CALL GET_NEXT_VALUE(TEMP_STR, MAP_INFO_PARAMS(I), N)
   END DO
! *****************************************************************************
...
! *****************************************************************************
SUBROUTINE GET_NEXT_VALUE(STR, VALUE, START_POS)
! *****************************************************************************
   IMPLICIT NONE
   ...
   IF (END_POS > 0) THEN
      VALUE = TRIM(STR(START_POS:START_POS+END_POS-2))
      START_POS = START_POS + END_POS
   ELSE
      VALUE = TRIM(STR(START_POS:))
      START_POS = LEN(STR) + 1
   ENDIF
! *****************************************************************************
END SUBROUTINE GET_NEXT_VALUE
! *****************************************************************************
