...
CHARACTER(400) :: FNTIF, FNBSQ, FNHDR
...
IF (USE_LANDSCAPE_FILE) THEN
   IF (IRANK_WORLD .EQ. 0) THEN
      ...
      IF (.NOT. HDR_EXISTS .OR. .NOT. BSQ_EXISTS) THEN
         SHELLSTR = '"' // TRIM(PATH_TO_GDAL) // 'gdal_translate" -q -of ENVI -co "INTERLEAVE=BSQ" "' // &
                    TRIM(FNTIF) // '" "' // TRIM(FNBSQ) // '"'
         IF (FEEDBACK_LEVEL .GE. 3) WRITE(*,'(A)') TRIM(SHELLSTR)
         CALL RUN_SHELL_COMMAND(SHELLSTR)
      ENDIF
   ENDIF
   CALL MPI_BARRIER(MPI_COMM_WORLD, IERR)
ENDIF
...
!      FLUSH(LUOUT)
      ...
      IF (CONVERT_TO_GEOTIFF_LOCAL) THEN
         ...
         CALL DELETE_FILE(TRIM(FNBIL))
         CALL DELETE_FILE(TRIM(FNHDR))
...
CHARACTER(400) :: FNHDR, FNBSQ, FNTIF, FNPRJ, FNXML, TEMPSTR, LINENOW
CHARACTER(:), ALLOCATABLE :: SHELLSTR
...
INTEGER :: I, IOS, IPOS, ITYPE, NLINES
LOGICAL :: XML_EXISTS
...
IF (DELETE_INTERMEDIATE_FILES) THEN
   CALL DELETE_FILE(TRIM(FNBSQ))
   CALL DELETE_FILE(TRIM(FNHDR))
   CALL DELETE_FILE(TRIM(FNPRJ))
   CALL DELETE_FILE(TRIM(FNXML))
ENDIF
...
INTEGER :: I, J, IOS, ITYPE, ITILE, JTILE, IDUMMY, NCOLS_TILE, NROWS_TILE
LOGICAL :: FOUND
...
RASTER%NCOLS = NCOLS_TILE * 3
RASTER%NROWS = NROWS_TILE * 3
...
RASTER%YLLCORNER = RASTER%YLLCORNER - RASTER%YDIM * NROWS_TILE
...
TYPE (RASTER_TYPE) :: RASTER
CHARACTER(400), INTENT (IN) :: INDIR, FN
CHARACTER(400) :: FNHDR, FNBSQ, FNTIF, TEMPSTR
CHARACTER(:), ALLOCATABLE :: SHELLSTR
