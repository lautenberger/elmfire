...
INTEGER*1, POINTER, DIMENSION(:,:) :: CROWN_FIRE

REAL :: XLLCORNER, YLLCORNER, CELLSIZE, DT, PERCENTILES(1:100)
REAL, POINTER, DIMENSION(:) :: T, STATS_ICASE_REAL, STATS_FIRE_AREA_SORTED, STATS_X_IGN, STATS_Y_IGN, STATS_TSTOP, &
                               STATS_FIRE_AREA
REAL, POINTER, DIMENSION(:,:) :: TOA, FLAME_LENGTH, VELOCITY_FPM

LOGICAL :: DUMP_HOURS_SINCE_BURNED, READ_ALREADY_BURNED, READ_PHI, HOURLY_PYREGENCE_OUTPUTS
...
NAMELIST /ELMFIRE_POST_INPUTS/ NX, NY, NCASES, XLLCORNER, YLLCORNER, CELLSIZE, OUTPUTS_DIRECTORY, DT, NUM_TIMESTEPS, &
                               ...
                               DUMP_TIME_OF_ARRIVAL, READ_ALREADY_BURNED, ALREADY_BURNED_FILENAME, READ_PHI, PHI_FILENAME, &
                               PATH_TO_GDAL, HOURLY_PYREGENCE_OUTPUTS, START_TIME_MINUTES_PAST_HOUR
...
!   THREED_ARRAY_SIZE=INT(NX*NY*NUM_TIMESTEPS,MPI_ADDRESS_KIND)*2_MPI_ADDRESS_KIND
      ...
      IF (NUM_CASES_TO_PROCESS .LT. 0) THEN !Use percentiles
         DO IPERCENTILES = 1, N_PERCENTILES
            ...
            IF (IRANK_WORLD .EQ. 0) THEN
               ...
               WRITE(*,*) 'ICASE: ', ICASE
               CALL POPULATE_RASTER_FROM_ELMFIRE_BINARY_FILE(FOUR, SIX)
               IF (DUMP_HOURS_SINCE_BURNED) CALL CALC_HOURS_SINCE_BURNED(NX, NY, NUM_TIMESTEPS)
            ENDIF

            CALL MPI_BARRIER(MPI_COMM_WORLD, IERR)
            CALL DUMP_ELMFIRE (ID, NUM_TIMESTEPS, NX, NY, XLLCORNER,YLLCORNER,CELLSIZE)
   ...
   ELSE !GRIDFIRE
      ...
      DO IPERCENTILES = 1, N_PERCENTILES
         ...
         IF (IRANK_WORLD .EQ. 0) THEN
            WRITE(*,*) 'Percentile spread: ', PERCENTILE_SPREAD
            WRITE(*,*) 'Acres: ', STATS_FIRE_AREA(ICASE)
            CALL POPULATE_RASTER_FROM_GRIDFIRE_BINARY_FILE(FOUR, SIX)
            IF (DUMP_HOURS_SINCE_BURNED) CALL CALC_HOURS_SINCE_BURNED(NX, NY, NUM_TIMESTEPS)
         ENDIF
...
! Dump flame length percentiles
            ...
            CALL WRITE_BIL_RASTER(RTEMP, OUTPUTS_DIRECTORY, FN, .FALSE., .FALSE.)
...
! And spread rate percentiles
         ...
         DO J = 0, 2
            ...
            CALL WRITE_BIL_RASTER(I2TEMP, OUTPUTS_DIRECTORY, FN, .FALSE., .FALSE.)
         ENDDO
...
! *****************************************************************************
SUBROUTINE POPULATE_RASTER_FROM_ELMFIRE_BINARY_FILE(FOUR, SIX)
! *****************************************************************************
...
FN = TRIM(OUTPUTS_DIRECTORY) // 'toa_' // FOUR // '_' // SIX // '.bin'
...
IF (DUMP_HOURS_SINCE_BURNED) THEN
   ...
   DO I = 1, NUM_BURNED
      ...
      TOA ( BINARY_OUTPUTS_IX(I), BINARY_OUTPUTS_IY(I) ) = BINARY_OUTPUTS_TOA (I)
   ENDDO
ENDIF
...
! *****************************************************************************
SUBROUTINE POPULATE_RASTER_FROM_GRIDFIRE_BINARY_FILE(FOUR, SIX)
! *****************************************************************************
...
IF (DUMP_HOURS_SINCE_BURNED) THEN
   TOA(:,:) = -9999.
   DO I = 1, NUM_BURNED
      TOA ( BINARY_OUTPUTS_IX_GRIDFIRE(I), BINARY_OUTPUTS_IY_GRIDFIRE(I) ) = BINARY_OUTPUTS_TOA (I)
   ENDDO
ENDIF
...
!WRITE(*,*) 'Starting HOURS_SINCE_BURNED loop'
...
!$OMP PARALLEL DO COLLAPSE(2) NUM_THREADS(NPROC) DEFAULT(NONE) PRIVATE(IX,IY) SHARED(IT,ALREADY_BURNED,PHI,HOURS_SINCE_BURNED,NUM_TIMESTEPS,NY,NX)
      DO IY = 1, NY
      DO IX = 1, NX
         IF (ALREADY_BURNED%R4(IX,IY,1) .GT.  1E-3 .AND. PHI%R4(IX,IY,1) .GT. -9998.) HOURS_SINCE_BURNED%I2(IX,IY,IT) = 200
      ENDDO
      ENDDO
!$OMP END PARALLEL DO
   ENDIF
...
!$OMP PARALLEL DO COLLAPSE(2) NUM_THREADS(NPROC) DEFAULT(NONE) PRIVATE(IX,IY) SHARED(IT,PHI,HOURS_SINCE_BURNED,NUM_TIMESTEPS,NY,NX)
      DO IY = 1, NY
      DO IX = 1, NX
         IF (PHI%R4(IX,IY,1) .LT. -1E-3 .AND. PHI%R4(IX,IY,1) .GT. -9998.) HOURS_SINCE_BURNED%I2(IX,IY,IT) = 201
      ENDDO
      ENDDO
!$OMP END PARALLEL DO
   ENDIF
...
! *****************************************************************************
SUBROUTINE DUMP_ELMFIRE(ID,NUM_TIMESTEPS,NX,NY,XLLCORNER,YLLCORNER,CELLSIZE)
! *****************************************************************************

INTEGER, INTENT(IN) :: NUM_TIMESTEPS, NX, NY
REAL, INTENT(IN) :: XLLCORNER,YLLCORNER,CELLSIZE
CHARACTER(6), INTENT(IN) :: ID
INTEGER :: IT
...
DO IT = 1, NUM_TIMESTEPS
   ...
   IF (DUMP_HOURS_SINCE_BURNED) THEN
      ...
      CALL WRITE_BIL_RASTER_THREADSAFE(SINGLEBAND_I2, OUTPUTS_DIRECTORY, FN, .FALSE., .FALSE., IRANK_WORLD)
   ENDIF
   ...
   IF (DUMP_TIME_OF_ARRIVAL .AND. IT .EQ. 1) THEN
      ...
      CALL WRITE_BIL_RASTER_THREADSAFE(SINGLEBAND_REAL, OUTPUTS_DIRECTORY, FN, .FALSE., .FALSE., IRANK_WORLD)
   ENDIF

   IF (DUMP_FLAME_LENGTH) THEN
      SINGLEBAND_REAL%R4(:,:,1) = SINGLEBAND_REAL%NODATA_VALUE
      WHERE (TOA(:,:) .GT. 0. .AND. TOA(:,:) .LE. T(IT) ) SINGLEBAND_REAL%R4(:,:,1) = FLAME_LENGTH(:,:)
   ...
   IF (DUMP_SPREAD_RATE) THEN
      SINGLEBAND_REAL%R4(:,:,1) = SINGLEBAND_REAL%NODATA_VALUE
      WHERE (TOA(:,:) .GT. 0. .AND. TOA(:,:) .LE. T(IT) ) SINGLEBAND_REAL%R4(:,:,1) = VELOCITY_FPM(:,:)
