...
USE ELMFIRE_VARS
USE ELMFIRE_SUBS, ONLY: RUN_SHELL_COMMAND, SPATIAL_ERROR, NAMELIST_GROUP_PRESENT
...
! contains gdal_translate from PATH using the host shell's lookup command.
! An explicit PATH_TO_GDAL in the namelist always takes precedence.
IF (TRIM(PATH_TO_GDAL) .EQ. 'auto') THEN
   ...
   IF (OPERATING_SYSTEM .EQ. 'windows') THEN
      CALL RUN_SHELL_COMMAND('where gdal_translate > "' // TRIM(GDALTMP) // '" 2>' // TRIM(NULL_DEVICE), EXITSTAT=IOS)
   ELSE
      CALL RUN_SHELL_COMMAND('command -v gdal_translate > "' // TRIM(GDALTMP) // '" 2>' // TRIM(NULL_DEVICE), EXITSTAT=IOS)
   ENDIF
   ...
   IF (IOS .EQ. 0) THEN
      ...
      CLOSE(LUGDAL, STATUS='DELETE')
   ENDIF

   ISLASH = INDEX(TRIM(GDALEXE), PATH_SEPARATOR, BACK=.TRUE.)
   IF (OPERATING_SYSTEM .EQ. 'windows') ISLASH = MAX(ISLASH, INDEX(TRIM(GDALEXE), '/', BACK=.TRUE.))
   IF (ISLASH .GT. 1) THEN
...
! An empty prefix lets the shell search PATH (especially for Windows wrappers).
...
IF (LEN_TRIM(A_SRS) >= LEN(A_SRS)) CALL SPATIAL_ERROR('A_SRS exceeds 4095 characters; use a CRS-file path')
...
COMPRESSION_ALGO, CONVERT_TO_GEOTIFF, DTDUMP, DUMP_AFFECTED_LAND_VALUE, DUMP_AFFECTED_POPULATION, DUMP_AFFECTED_REAL_ESTATE_VALUE, &
...
DUMP_TIMES_CSV, DUMP_TRANSIENT_ACREAGE, DUMP_VELOCITY, DUMP_WD20, DUMP_CFFDRS_DEBUG, DUMP_WS20, EMBER_COUNT_BIN_LO, EMBER_COUNT_BIN_HI, &
...
DUMP_HRR_TRANSIENT, TIME_AT_BURNED_ACRES, USE_FOUR_DIGITS_IN_IWX_BAND, VIRTUAL_STATION_X, VIRTUAL_STATION_Y, DUMP_EVERY_STEP, ZLEVEL
...
SUNRISE_HOUR                  = -1.0 ! UTC; negative requests automatic calculation when diurnal is enabled
SUNSET_HOUR                   = -1.0 ! UTC, unwrapped: Hawaiian sunset can exceed 24 hours
TARGET_CFL                    = 0.4
...
! Compatibility only: the raster always supplies grid geometry.
INTEGER :: IOS
CHARACTER(256) :: IOSMSG
CHARACTER(:), ALLOCATABLE :: INPUTS_SRS
REAL :: COMPUTATIONAL_DOMAIN_CELLSIZE, COMPUTATIONAL_DOMAIN_XLLCORNER, COMPUTATIONAL_DOMAIN_YLLCORNER
NAMELIST /COMPUTATIONAL_DOMAIN/ A_SRS, COMPUTATIONAL_DOMAIN_CELLSIZE, &
   COMPUTATIONAL_DOMAIN_XLLCORNER, COMPUTATIONAL_DOMAIN_YLLCORNER

IF (.NOT. NAMELIST_GROUP_PRESENT('COMPUTATIONAL_DOMAIN')) RETURN
INPUTS_SRS = TRIM(A_SRS)
A_SRS = ''
COMPUTATIONAL_DOMAIN_CELLSIZE = -HUGE(1.0)
COMPUTATIONAL_DOMAIN_XLLCORNER = -HUGE(1.0)
COMPUTATIONAL_DOMAIN_YLLCORNER = -HUGE(1.0)
READ(LUINPUT,NML=COMPUTATIONAL_DOMAIN,IOSTAT=IOS,IOMSG=IOSMSG)
IF (IOS /= 0) CALL SPATIAL_ERROR('Reading &COMPUTATIONAL_DOMAIN: ' // TRIM(IOSMSG))
IF (LEN_TRIM(A_SRS) >= LEN(A_SRS)) CALL SPATIAL_ERROR('Legacy A_SRS exceeds 4095 characters; use a CRS-file path')
IF (LEN(INPUTS_SRS) > 0) A_SRS = INPUTS_SRS
IF (IRANK_WORLD == 0 .AND. (COMPUTATIONAL_DOMAIN_CELLSIZE /= -HUGE(1.0) .OR. &
    COMPUTATIONAL_DOMAIN_XLLCORNER /= -HUGE(1.0) .OR. COMPUTATIONAL_DOMAIN_YLLCORNER /= -HUGE(1.0))) &
   WRITE(*,*) 'Ignoring legacy COMPUTATIONAL_DOMAIN geometry; using raster metadata.'
