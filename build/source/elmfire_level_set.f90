...
LOGICAL :: IA_HAS_OCCURRED, LOPEN, DUMPEDYET, GO, CALL_SPOTTING, JUST_INTERPOLATED, DUMP_SMOKE_OUTPUTS, RUN
...
IF (.NOT. RANDOM_IGNITIONS) THEN
   ...
   DO IX = 1, NX
      IF (PHIP(IX,IY) .LE. 0.) THEN
         ...
         LIST_BURNED%TAIL%FLIN_SURFACE           = 2200
         LIST_BURNED%TAIL%TIME_OF_ARRIVAL        = T
         LIST_BURNED%TAIL%WS20_NOW               = 15
         ...
         DO IY = 1,NY
            DO IX = 1,NX
               ...
               IF (USE_EMBER_IGNITION_MODEL) THEN
                  IF (.NOT. EMBER_IGNITION(IX,IY,T)) CYCLE
               ELSE
                  ! IF (EMBER_TIGN(IX,IY) .GT. 0) WRITE(*,*) 'SURFACE_FIRE(IX,IY)', SURFACE_FIRE(IX,IY), 'PHIP(IX,IY)', PHIP(IX,IY), 'EMBER_TIGN(IX,IY)',EMBER_TIGN(IX,IY)
   ...
   IF (NUM_TIME_AT_BURNED_ACRES .GT. 0) THEN
      ...
      DO I = 1, NUM_TIME_AT_BURNED_ACRES
          ...
          IF (T .GT. TSTOP .AND. STATS_SIMULATION_TSTOP_HOURS(ICASE) .GT. 0) THEN
             WRITE(LUBAT+IRANK_WORLD, 777) ICASE, TIME_AT_BURNED_ACRES(I), STATS_SIMULATION_TSTOP_HOURS(ICASE)
          ELSE
             WRITE(LUBAT+IRANK_WORLD, 777) ICASE, TIME_AT_BURNED_ACRES(I), T/3600.0
          ENDIF
      ENDDO
   ENDIF
777 FORMAT (I7, ',', F8.1, ',', F7.2)
...
IF (DUMP_EMBER_FLUX_UMD) THEN
   ...
   DO ITIMESTEP=1,EMBER_FLUX_TABLE_LEN
         ...
         DO K = 1, LIST_BURNED%NUM_NODES
            ...
            DO I = MAX(1,IX-1), MIN(NX,IX+1)
               IF ( (FBFM%I2(I,J,1) .EQ. 91) .AND. (POPULATION_DENSITY%R4(I,J,1) .GT. 0.) ) THEN
                  ...
                  CALL APPEND(LIST_BURNED, I, J, .TRUE., T)
