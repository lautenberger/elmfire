...
INTEGER :: I, ILOC, J, IX, IY, ITIMESTEP, IX_IGN, IY_IGN, ISTEP, ISTATION, K, LU, IT1, IT2, &
...
REAL :: T_LAST_EXTENDED_ATTACK, T_LAST_INTERPOLATE_M1, T_LAST_INTERPOLATE_M10, T_LAST_INTERPOLATE_M100, &
        ...
        T_LAST_SMOKE_OUTPUT, QDOTNOW, XCEN, YCEN, THI, TLO, MDOTNOW, RADIUS, WN_FUEL
...
! Point pointers to analysis rasters:
   ...
   IF (DUMP_EMBER_FLUX) EMBER_FLUX%I2(:,:,:) = 0
...
IF (RANDOM_IGNITIONS) THEN
   ...
   IF (USE_PYROMES .AND. DURATION_PDF_BY_PYROME) THEN
      IPYROME = PYROMES%I2(IX_IGN,IY_IGN,1)
      CALL RANDOM_NUMBER(R0)
...
IF (DUMP_EMBER_FLUX .AND. (.NOT. ACCUMULATE_EMBER_FLUX) ) EMBER_FLUX%I2(:,:,:) = 0
...
IF (.NOT. RANDOM_IGNITIONS) THEN
   ...
   DO IX = 1, NX
      IF (PHIP(IX,IY) .LE. 0.) THEN
         ...
         ICOUNT = ICOUNT + 1
         CALL APPEND(LIST_BURNED, IX, IY, .TRUE., T)

         LIST_BURNED%TAIL%FLIN_SURFACE           = 1E-6
         LIST_BURNED%TAIL%TIME_OF_ARRIVAL        = T
         LIST_BURNED%TAIL%WS20_NOW               = 1E-6
...
IF (USE_UMD_SPOTTING_MODEL .AND. USE_SPOTTING_LOOKUP_TABLE) THEN
   N_SPOT_FIRES = 0
...
! Note that per Thomas (1963) and Rothermel (1991), crown fire flame length is Lf=0.2*I^2/3
         ...
         IF (ENABLE_SPOTTING) THEN
            ...
            IF (.NOT. USE_UMD_SPOTTING_MODEL) THEN
               ...
               IF (ENABLE_SURFACE_FIRE_SPOTTING .AND. (.NOT. CALL_SPOTTING) ) THEN
                  IF (C%FLIN_SURFACE .GT. CRITICAL_SPOTTING_FIRELINE_INTENSITY(FBFM%I2(C%IX,C%IY,1))) THEN
                     CALL RANDOM_NUMBER(R0)
   ...
   IF (USE_UMD_SPOTTING_MODEL) THEN
      ...
      DO I = 1, LIST_BURNED%NUM_NODES
         ...
         IF (C%TAU_EMBERGEN .LT. TAU_EMBERGEN) THEN
            IF (C%FLIN_SURFACE .GT. CRITICAL_SPOTTING_FIRELINE_INTENSITY(FBFM%I2(C%IX,C%IY,1))) THEN
               CALL RANDOM_NUMBER(R0)
               IF (R0 .LT. 0.01*SURFACE_FIRE_SPOTTING_PERCENT(FBFM%I2(C%IX,C%IY,1))) CALL_SPOTTING = .TRUE. 
               CONTINUE
...
!Dump main GIS outputs:
   IF (ABS(T - DUMPTIMES(IDUMPCOUNT)) .LE. 0.5*DT .OR. T .GE. DUMPTIMES(IDUMPCOUNT) .OR. T .GE. TSTOP) THEN !CWL
      CALL MAIN_DUMP_ROUTINE(IDUMPCOUNT, NDUMPS, ICASE, T, DUMPEDYET, ACRES)
!      CALL NEW_DUMP_ROUTINE(LIST_TAGGED,'tagged_time_added',T,'time_added',ICASE) 
   ENDIF

!   IF (T .GE. TSTOP) THEN
!      CALL NEW_DUMP_ROUTINE(LIST_SUPPRESSED,'time_suppressed',T,'time_suppressed',ICASE) 
!      CALL NEW_DUMP_ROUTINE(LIST_BURNED,'time_of_arrival',T,'time_of_arrival',ICASE) 
!   ENDIF
...
! Close virtual stations files:
IF (NUM_VIRTUAL_STATIONS .GT. 0 .AND. IRANK_WORLD .EQ. 0) THEN
   DO ISTATION = 1, NUM_VIRTUAL_STATIONS
      LU = LUOUTPUT + ISTATION
...
! Calculate fire area
IF (DUMP_FIRE_SIZE_STATS .OR. CALCULATE_BURN_PROBABILITY) THEN
   STATS_SURFACE_FIRE_AREA(ICASE) = 0.
   STATS_CROWN_FIRE_AREA  (ICASE) = 0.
   STATS_FIRE_VOLUME      (ICASE) = 0.
   ...
   DO I = 1, LIST_BURNED%NUM_NODES
      ...
      IF (RANDOM_IGNITIONS .AND. CALCULATE_BURN_PROBABILITY) THEN
         IF (RANDOM_IGNITIONS_TYPE .EQ. 1) THEN
            A_TIMES_BURNED(IX,IY,1) = A_TIMES_BURNED(IX,IY,1) + STATS_PROB(ICASE)
         ELSE
            A_TIMES_BURNED(IX,IY,1) = A_TIMES_BURNED(IX,IY,1) + 1.0
         ENDIF
      ENDIF
      IF (C%CROWN_FIRE .GT. 0) THEN
         STATS_CROWN_FIRE_AREA(ICASE) = STATS_CROWN_FIRE_AREA(ICASE) + 1.
         IF (RANDOM_IGNITIONS) THEN
            IF (CALCULATE_BURN_PROBABILITY) THEN
               IF (C%CROWN_FIRE .EQ. 1) A_TIMES_BURNED(IX,IY,2) = A_TIMES_BURNED(IX,IY,2) + STATS_PROB(ICASE)
               IF (C%CROWN_FIRE .EQ. 2) A_TIMES_BURNED(IX,IY,3) = A_TIMES_BURNED(IX,IY,3) + STATS_PROB(ICASE)
            ENDIF
         ELSE
            IF (CALCULATE_BURN_PROBABILITY) THEN
               IF (C%CROWN_FIRE .EQ. 1) A_TIMES_BURNED(IX,IY,2) = A_TIMES_BURNED(IX,IY,2) + 1.0
               IF (C%CROWN_FIRE .EQ. 2) A_TIMES_BURNED(IX,IY,3) = A_TIMES_BURNED(IX,IY,3) + 1.0
            ENDIF
         ENDIF
      ENDIF   
      C => C%NEXT
      ...
      IF (ESTIMATE_URBAN_LOSSES) THEN
         ...
         DO K = 1, LIST_BURNED%NUM_NODES
            ...
            DO I = MAX(1,IX-1), MIN(NX,IX+1)
               IF (SURFACE_FIRE(I,J) .GT. 0 .AND. FBFM%I2(I,J,1) .EQ. 91) THEN
                  SURFACE_FIRE(I,J) = 1
...
!WRITE(*,*) 'SIMULATION_TSTOP_HOURS : ', SIMULATION_TSTOP_HOURS
...
!      IF ( TOA(IX,IY) .GT. 0. .AND. T - TOA(IX,IY) .GT. 20*DT ) UNTAG_BECAUSE_BURNED = .TRUE. 
   ...
   IF (C%TIME_SUPPRESSED .GT. 0.) THEN
      CALL APPEND(LIST_SUPPRESSED, IX, IY, .FALSE., T)
...
REAL :: PHIMAG, PHIWX, PHIWY, PHIX, PHIY, WSMFEFF, A, B, COSANG, SINANG, BOH, DXDT, DYDT, DENOM, &
        ...
        RPHIMAG, RDENOM, SQRT_LOW2_M1, STRUCTURE_AREA, SIGN_X, SIGN_Y
...
!      OPEN(20071205, FILE="UCBellips_DIAGNOSTIC.csv")
!      WRITE(20071205,*) LB_P%IFBFM, LB_P%ELLIPSE_PARAMETERS%DIST_DOWNWIND, LB_P%ELLIPSE_PARAMETERS%DIST_UPWIND, LB_P%ELLIPSE_PARAMETERS%DIST_SIDEWIND, LB_P%ELLIPSE_PARAMETERS%FOREST_FACTOR, &
!                        LB_P%ELLIPSE_PARAMETERS%ELLIPSE_MAJOR, LB_P%ELLIPSE_PARAMETERS%ELLIPSE_MINOR, LB_P%ELLIPSE_PARAMETERS%ELLIPSE_ECCENTRICITY
!      !WRITE(*,*) 'UCB CALLED'
      LB_P => LB_P%NEXT
   ENDDO
ENDIF
...
! Calculate length over width:
            C%LOW = MIN( 0.936*EXP(0.2566*WSMFEFF*60./5280.) + 0.461*EXP(-0.1548*WSMFEFF*60.0/5280.) - 0.397, MAX_LOW)
...
!            WRITE(777,*) C%IX, C%IY, C%BURNED, C%TIME_OF_ARRIVAL, T, C%HRR_TRANSIENT, C%FLIN_SURFACE
