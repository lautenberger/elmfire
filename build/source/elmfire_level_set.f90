...
INTEGER :: I, ILOC, J, IX, IY, ITIMESTEP, IX_IGN, IY_IGN, ISTEP, K, LU, IT1, IT2, &
           ...
           N_TO_TAG, N_SPOT_FIRES, ILH, IT2_LSP, IFBFM, IBLDGFM, ICOL, IROW, YEAR, MONTH, DAY_OF_MONTH
...
REAL :: T_LAST_EXTENDED_ATTACK, T_LAST_INTERPOLATE_M1, T_LAST_INTERPOLATE_M10, T_LAST_INTERPOLATE_M100, &
        ...
        CELLTIME, WET_WOOD_CALORIFIC_VALUE, CELLENERGYRELEASE, PM_FLAMING_MAX, PM_SMOLDERING_MAX, TOTALENERGY
...
! Extended attack model
   ...
   IF (ENABLE_SMOKE_OUTPUTS .AND. T - T_LAST_SMOKE_OUTPUT .GE. DT_SMOKE_OUTPUTS ) THEN
      C => LIST_BURNED%HEAD
      DO I = 1, LIST_BURNED%NUM_NODES
         IF (C%TIME_IGNITED .GE. T_LAST_SMOKE_OUTPUT) THEN
            CELLTIME = C%TIME_IGNITED - T_LAST_SMOKE_OUTPUT 
            WET_WOOD_CALORIFIC_VALUE = DRY_WOOD_CALORIFIC_VALUE*(1-C%M10)-2.44*C%M10
            CELLENERGYRELEASE = (C%FLIN_SURFACE + C%FLIN_CANOPY) * ASP%CELLSIZE**2 / (C%VELOCITY * 0.3048 / 60)
            ...
            ELSE IF (CELLTIME .LE. DT_SMOKE_OUTPUTS - FLAMING_TIME) THEN ! STILL SMOLDERING
               STATS_PM2P5_RELEASE(ICASE) = STATS_PM2P5_RELEASE(ICASE) + PM_FLAMING_MAX + PM_SMOLDERING_MAX * &
                  (DT_SMOKE_OUTPUTS - CELLTIME) / SMOLDERING_TIME
               TOTALENERGY = TOTALENERGY + CELLENERGYRELEASE * (1 + (DT_SMOKE_OUTPUTS - CELLTIME) / SMOLDERING_TIME)
            ELSE ! STILL FLAMING
               STATS_PM2P5_RELEASE(ICASE) = STATS_PM2P5_RELEASE(ICASE) + PM_FLAMING_MAX * &
                  (DT_SMOKE_OUTPUTS - CELLTIME) / FLAMING_TIME
               TOTALENERGY = TOTALENERGY + CELLENERGYRELEASE * (DT_SMOKE_OUTPUTS - CELLTIME) / SMOLDERING_TIME
            ENDIF

            XCEN = XCEN + CELLENERGYRELEASE * C%IX
            YCEN = YCEN + CELLENERGYRELEASE * C%IY

            IF (CELLENERGYRELEASE .GT. 0.) THEN
               IXCEN = NINT(XCEN/CELLENERGYRELEASE)
               IYCEN = NINT(YCEN/CELLENERGYRELEASE)
            ELSE
               IXCEN = IX_IGN
               IYCEN = IY_IGN
               CELLENERGYRELEASE = 0
      ...
      IF (DUMP_EMITIMES) THEN
         ...
         IF (.NOT. LOPEN) THEN
            ...
            WRITE(LUEMIT+IRANK_WORLD,'(I4,1X,I2.2,1X,I2.2,1X,I2.2,1X,I4.4,1X,I4.4)') &
               YEAR, MONTH, DAY_OF_MONTH, HOUR_OF_DAY,                            &
               SIMULATION_TSTOP_HOURS, NINT(SIMULATION_TSTOP_HOURS*3600/DT_SMOKE_OUTPUTS)

         ENDIF 

         WRITE(LUEMIT+IRANK_WORLD, &
            '(I4, 1X, I2.2, 1X, I2.2, 1X, I2.2, 1X, "00" , 1X, I2.2, I2.2, 1X, F8.3, 1X, F9.3, 1X, F6.1, 1X, 3ES11.3)')     &
            YEAR, MONTH, DAY_OF_MONTH, HOUR_OF_DAY, NINT(DT_SMOKE_OUTPUTS/3600), MOD(NINT(DT_SMOKE_OUTPUTS/60),60),  &
            ASP%XLLCORNER + XCEN, ASP%YLLCORNER + YCEN, DEM%R4(NINT(XCEN),NINT(YCEN),1),                                               &
