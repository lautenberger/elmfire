...
IF (.NOT. RANDOM_IGNITIONS) THEN
   ...
   DO IX = 1, NX
      IF (PHIP(IX,IY) .LE. 0.) THEN
         ...
         LIST_BURNED%TAIL%TAU_EMISSION           = 0.
...
! Note that per Thomas (1963) and Rothermel (1991), crown fire flame length is Lf=0.2*I^2/3
   ...
   IF (USE_UMD_SPOTTING_MODEL) THEN
      ...
      DO I = 1, LIST_BURNED%NUM_NODES
         IF (USE_PHYSICAL_SPOTTING_DURATION) TAU_EMISSION = C%LOCAL_EMISSION_DURATION
         IF (C%TAU_EMISSION .LT. TAU_EMISSION) THEN
            IF (C%FLIN_SURFACE .GT. CRITICAL_SPOTTING_FIRELINE_INTENSITY(FBFM%I2(C%IX,C%IY,1))) THEN
               CALL RANDOM_NUMBER(R0)
               IF (R0 .LT. 0.01*SURFACE_FIRE_SPOTTING_PERCENT(FBFM%I2(C%IX,C%IY,1))) CALL_SPOTTING = .TRUE. 
               CONTINUE
            ...
            IF (CALL_SPOTTING) THEN
               ...
               CALL SPOTTING ( C%IX,C%IY,C%WS20_NOW,C%FLIN_SURFACE,F_METEOROLOGY,WS20_LO,WS20_HI,WD20_LO, &
                               WD20_HI,N_SPOT_FIRES,IX_SPOT_FIRE,IY_SPOT_FIRE,ICASE,DT,T, C%TAU_EMISSION,&
                               SOURCE_FUEL_IGN_MULT (FBFM%I2(C%IX,C%IY,1)), &
                               FOOTPRINT_PERCENTAGE, C%FMC, C%IFBFM, WN_FUEL) ! Parameters added to calculate number of physical embers
            ENDIF
            ! CALL SPOTTING ( C%IX,C%IY,C%WS20_NOW,C%FLIN_SURFACE,F_METEOROLOGY, WS20_LO,WS20_HI,WD20_LO, &
            ! WD20_HI,N_SPOT_FIRES,IX_SPOT_FIRE,IY_SPOT_FIRE,ICASE,DT,T,C%TAU_EMISSION,1.0 )
         ENDIF
         C%TAU_EMISSION = MIN (TAU_EMISSION, C%TAU_EMISSION + DT)
         C => C%NEXT
      ENDDO
   ENDIF
         ...
         DO I = 1, NUM_TRACKED_EMBERS
            ...
            IF (SURFACE_FIRE(IX,IY) .LE. 0 .AND. ADJ%R4(IX,IY,1) .GT. 0. .AND. (.NOT. ISNONBURNABLE(IX,IY) ) ) THEN
               ...
               IF (DUMP_SPOTTING_OUTPUTS) THEN
                  FN = TRIM(OUTPUTS_DIRECTORY) // 'spotting_stats_' // SIX_ICASE // '.csv'
...
!      MDOTNOW = SMOKE_YIELD * QDOTNOW / SMOKE_HOC
...
! Determine effective radius
      ...
      IF (DUMP_SMOKE_OUTPUTS) THEN
         FN = TRIM(OUTPUTS_DIRECTORY) // 'smoke_' // SIX_ICASE // '.csv'
         ...
         IF (.NOT. LOPEN) THEN
            ...
            WRITE(LUSMOKE+IRANK_WORLD,'(A)') 't (s), xcen (m), ycen (m), area (m2), HRR (W), mdotsmoke (ug/h)'
         ENDIF

         WRITE(LUSMOKE+IRANK_WORLD,999) T, XCEN, YCEN, ACRES*4046.86, 1E3*QDOTNOW, 3.6E12*MDOTNOW
      ENDIF

      T_LAST_SMOKE_OUTPUT = T - 0.5 * DT
      
   ENDIF ! ENABLE_SMOKE_OUTPUTS

998 FORMAT(I9.1,',',I9.1,',',I9.1,',',I9.1,',',F12.2,',',F12.2)
999 FORMAT(F9.1,',',F10.1,',',F10.1,',',E12.5,',',E12.5,',',E12.5)
...
IF (DUMP_BINARY_OUTPUTS) THEN
   ...
   FN = TRIM(OUTPUTS_DIRECTORY) // 'toa_' // FOUR_IWX_BAND // '_' // SIX_ICASE // '.bin'
...
! Rotate based on direction of maximum spread:
         ...
         IF (USE_UMD_SPOTTING_MODEL .AND. USE_PHYSICAL_SPOTTING_DURATION) C%LOCAL_EMISSION_DURATION = FUEL_MODEL_TABLE_2D(C%IFBFM,ILH)%TR
