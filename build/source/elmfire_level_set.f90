...
INTEGER :: I, ILOC, J, IX, IY, ITIMESTEP, IX_IGN, IY_IGN, ISTEP, K, LU, IT1, IT2, &
           ...
           N_TO_TAG, N_SPOT_FIRES, ILH, IT2_LSP
...
REAL :: T_LAST_EXTENDED_ATTACK, T_LAST_INTERPOLATE_M1, T_LAST_INTERPOLATE_M10, T_LAST_INTERPOLATE_M100, &
        ...
        T_LAST_SMOKE_OUTPUT, QDOTNOW, XCEN, YCEN, THI, TLO, MDOTNOW, RADIUS, WN_FUEL, RUNTIME, T_TOTAL
   ...
   IF (ENABLE_SPOTTING .AND. USE_UMD_SPOTTING_MODEL) THEN
      ...
      DO I = 1, LIST_BURNED%NUM_NODES
         IF (USE_PHYSICAL_SPOTTING_DURATION) THEN
            IF (C%T_START_SPOTTING .LT. 1E-3) THEN
               IF (C%IFBFM .EQ. 91 ) THEN
                  IF (C%HRR_TRANSIENT .GE. CRITICAL_SPOTTING_FIRELINE_INTENSITY(FBFM%I2(C%IX,C%IY,1))/ANALYSIS_CELLSIZE ) THEN
                     C%T_START_SPOTTING = SQRT(CRITICAL_SPOTTING_FIRELINE_INTENSITY(FBFM%I2(C%IX,C%IY,1))/1000.0)*BUILDING_FUEL_MODEL_TABLE(C%IBLDGFM)%T_1MW
                     C%T_START_SPOTTING = MIN(C%T_START_SPOTTING,BUILDING_FUEL_MODEL_TABLE(C%IBLDGFM)%T_FULLDEV)
                     C%T_START_SPOTTING = MAX(C%T_START_SPOTTING,0.0)
                     C%T_START_SPOTTING = C%T_START_SPOTTING + T
                     ...
                     T_TOTAL = BUILDING_FUEL_MODEL_TABLE(C%IBLDGFM)%T_DECAY+ &
                              ...
                              ANALYSIS_CELLSIZE*ANALYSIS_CELLSIZE/BUILDING_FUEL_MODEL_TABLE(C%IBLDGFM)%T_DECAY

                     C%T_END_SPOTTING = T_TOTAL-CRITICAL_SPOTTING_FIRELINE_INTENSITY(FBFM%I2(C%IX,C%IY,1))/BUILDING_FUEL_MODEL_TABLE(C%IBLDGFM)%HRRPUA_PEAK* & 
                                       (T_TOTAL-BUILDING_FUEL_MODEL_TABLE(C%IBLDGFM)%T_DECAY);
                     ...
                     C%T_END_SPOTTING = C%T_END_SPOTTING+T
                  ENDIF
               ENDIF
            ELSE
