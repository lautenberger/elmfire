...
INTEGER :: I, ILOC, J, IX, IY, ITIMESTEP, IX_IGN, IY_IGN, ISTEP, ISTATION, K, LU, IT1, IT2, &
           ...
           N_TO_TAG, N_SPOT_FIRES, ILH, EMBER_FLUX_TABLE_LEN
...
IF (USE_EULERIAN_FRAMEWORK) THEN
    ...
    ALLOCATE (SPOTTING_TIME_TABLE(1:EMBER_FLUX_TABLE_LEN))
    DO I = 1, EMBER_FLUX_TABLE_LEN
        SPOTTING_TIME_TABLE(I) = (REAL(I)-1.0)*DT_DUMP_EMBER_FLUX
    ENDDO
    ALLOCATE (EMBER_ACCUMULATION_RATE(1:NX, 1:NY, 1:EMBER_FLUX_TABLE_LEN))
ENDIF
...
! Note that per Thomas (1963) and Rothermel (1991), crown fire flame length is Lf=0.2*I^2/3
   ...
   IF (USE_UMD_SPOTTING_MODEL) THEN
      IF (USE_EULERIAN_FRAMEWORK) THEN
         DO IY = 1,NY
            DO IX = 1,NX
               IF (EMBER_TIGN(IX,IY) .GT. T+DT .OR. EMBER_TIGN(IX,IY) .LT. 0) CYCLE
...
IF (DUMP_EMBER_FLUX_UMD) THEN
   INQUIRE(UNIT=713,OPENED=LOPEN)

   FN = TRIM(OUTPUTS_DIRECTORY) // 'ember_flux_hist_' // SEVEN_ICASE // '.csv'
   INQUIRE(UNIT=713,OPENED=LOPEN)
   IF (.NOT. LOPEN) THEN
      OPEN(713,FILE=TRIM(FN),FORM='FORMATTED',STATUS='REPLACE',IOSTAT=IOS)
   ENDIF

   DO ITIMESTEP=1,SIZE(SPOTTING_TIME_TABLE)
      WRITE(713,*) EMBER_ACCUMULATION_RATE(:,:,ITIMESTEP)
   ENDDO
