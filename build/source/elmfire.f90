...
! Initialize clocks before reading DUMP_TIMINGS; IT1 also supplies the randomized seed.
...
CALL MPI_BARRIER(MPI_COMM_WORLD, IERR)
IF (DUMP_TIMINGS) CALL ACCUMULATE_CPU_USAGE(2, IT1, IT2)
...
IF (DUMP_TIMINGS) CALL ACCUMULATE_CPU_USAGE(3, IT1, IT2)
...
CALL MPI_BARRIER(MPI_COMM_WORLD, IERR)
IF (DUMP_TIMINGS) CALL ACCUMULATE_CPU_USAGE(4, IT1, IT2)
...
ENDIF
   ...
   CALL MPI_BARRIER(MPI_COMM_WORLD, IERR)
   IF (DUMP_TIMINGS) CALL ACCUMULATE_CPU_USAGE(5, IT1, IT2)
...
CALL MPI_BARRIER(MPI_COMM_WORLD, IERR)
IF (DUMP_TIMINGS) CALL ACCUMULATE_CPU_USAGE(6, IT1, IT2)
...
CALL MPI_BARRIER(MPI_COMM_WORLD, IERR)
IF (DUMP_TIMINGS) CALL ACCUMULATE_CPU_USAGE(7, IT1, IT2)
...
CALL MPI_BARRIER(MPI_COMM_WORLD, IERR)
IF (DUMP_TIMINGS) CALL ACCUMULATE_CPU_USAGE(8, IT1, IT2)

IF (IRANK_WORLD .EQ. 0) WRITE(*,*) 'Setting up shared memory, part 2'
CALL SETUP_SHARED_MEMORY_2

CALL MPI_BARRIER(MPI_COMM_WORLD, IERR)
IF (DUMP_TIMINGS) CALL ACCUMULATE_CPU_USAGE(9, IT1, IT2)
...
CALL MPI_BARRIER(MPI_COMM_WORLD, IERR)
IF (DUMP_TIMINGS) CALL ACCUMULATE_CPU_USAGE(10, IT1, IT2)
...
CALL MPI_BARRIER(MPI_COMM_WORLD, IERR)
IF (DUMP_TIMINGS) CALL ACCUMULATE_CPU_USAGE(11, IT1, IT2)
...
IF (MODE .NE. 1) THEN
   ...
   DO IY = 1, ANALYSIS_NROWS
      ...
      DO IX = 1, ANALYSIS_NCOLS
         ...
         ! Apply the raw MODE=2 mask before allocating potential nodes.
         IF (MODE .EQ. 2 .AND. USE_IGNITION_MASK) THEN
            IF (IEEE_IS_NAN(IGN_MASK%R4(IX,IY,1))) CYCLE
            IF (IGN_MASK%R4(IX,IY,1) .LT. 1.0) CYCLE
            IF (.NOT. IEEE_IS_NAN(IGN_MASK%NODATA_VALUE)) THEN
               IF (IGN_MASK%R4(IX,IY,1) .EQ. IGN_MASK%NODATA_VALUE) CYCLE
            ENDIF
         ENDIF
...
IF (MODE .NE. 2) THEN
   ...
   DO WHILE (DONE_CASES .LT. NUM_CASES_TOTAL)
      ...
      IF (DUMP_TIMINGS) CALL SYSTEM_CLOCK(IT1)
      IF (DUMP_TIMINGS) CALL ACCUMULATE_CPU_USAGE(12, IT1, IT2)
      ...
      IF (DUMP_TIMINGS) CALL ACCUMULATE_CPU_USAGE(13, IT1, IT2)
   
      IF (NUM_MONTE_CARLO_VARIABLES .GT. 0 ) CALL RANDOM_NUMBER(COEFFS(:))
      IF (NUM_RASTERS_TO_PERTURB .GT. 0) CALL PERTURB_RASTERS(COEFFS(:))

      IF (DUMP_TIMINGS) CALL ACCUMULATE_CPU_USAGE(14, IT1, IT2)
      ...
      IF (DUMP_TIMINGS) CALL ACCUMULATE_CPU_USAGE(15, IT1, IT2)
   
      ! Case statistics and acreage milestones need a fresh start even without profiling.
      IF (DUMP_TIMINGS) THEN
         IT1_LSP = IT1
      ELSE
         CALL SYSTEM_CLOCK(IT1_LSP)
      ENDIF
      ...
      if (.not. IS_VIRTUAL_RUN) then
         ...
         IF (DUMP_TIMINGS) TIMINGS(IRANK_HOST+1,80) = TIMINGS(IRANK_HOST+1,80) + STATS_WALL_CLOCK_TIME(ICASE)
         IF (DUMP_TIMINGS) CALL SYSTEM_CLOCK(IT1)
      ...
      IF (IRANK_WORLD .EQ. 0 .AND. NPROC .GT. 1) THEN
         ...
         DO WHILE (TOTALCASESRUN .LT. (DONE_CASES - DONE_CASES_PREV - 1))
            ...
            IF (DUMP_TIMINGS) CALL ACCUMULATE_CPU_USAGE(17, IT1, IT2)
            ...
            IF (DUMP_TIMINGS) CALL ACCUMULATE_CPU_USAGE(18, IT1, IT2)
         ...
         IF (DUMP_TIMINGS) CALL ACCUMULATE_CPU_USAGE(19, IT1, IT2)

         ENDDO
      ENDIF
      IF (DUMP_TIMINGS) CALL ACCUMULATE_CPU_USAGE(16, IT1, IT2)
   ENDDO

   CALL MPI_BARRIER(MPI_COMM_WORLD, IERR)

   IF (DUMP_TIMINGS) CALL ACCUMULATE_CPU_USAGE(21, IT1, IT2)

   CALL POSTPROCESS()

   IF (DUMP_TIMINGS) CALL ACCUMULATE_CPU_USAGE(22, IT1, IT2)
