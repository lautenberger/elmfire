...
INTEGER :: COLOR, I, IASP, IBAND, IBIN, ICASE, ICASE_RECV, ICOL, IDEST=0, IERR=0, IOS, &
           ...
           IWX_BAND_TO_DUMP, IRANK_FROM, IROW, J, IT1, IT2, IX, IY, IWD20_TIMES10, M, N, &
...
CHARACTER(4) :: SUFFIX
CHARACTER(60) :: PREFIX, VERSIONSTRING='ELMFIRE 2023.0930'
CHARACTER(400) :: FN, MESSAGESTR

TYPE (RASTER_TYPE), POINTER :: R

TYPE(RASTER_TYPE) SPREAD_RATE_TO_DUMP, FLAME_LENGTH_TO_DUMP
...
IF (MODE .NE. 2) THEN
   IF (IRANK_WORLD .EQ. 0) WRITE(*,*) 'Calculating NUM_CASES_TOTAL'
...
IF (MODE .NE. 1) THEN
   ...
   DO IY = 1, ANALYSIS_NROWS
      ...
      DO IX = 1, ANALYSIS_NCOLS
         ...
         CALL APPEND(LIST_FIRE_POTENTIAL, IX, IY, .FALSE., 0.)
      ENDDO
   ENDDO
   ...
   DO IWX_BAND = METEOROLOGY_BAND_START, METEOROLOGY_BAND_STOP
      ...
      DO J = 1, 2
         ...
         DO I = 1, LIST_FIRE_POTENTIAL%NUM_NODES
            ...
            IF (J .EQ. 2) THEN
               ...
               IF (CROWN_FIRE_MODEL .GT. 0 .AND. C%FLIN_SURFACE .GE. C%CRITICAL_FLIN) C%FLIN_CANOPY = C%HPUA_CANOPY * C%VELOCITY_DMS * 5.08E-3
      ...
      IF (DUMP_FLAME_LENGTH) THEN
         ...
         CALL WRITE_BIL_RASTER_THREADSAFE(FLAME_LENGTH_TO_DUMP,OUTPUTS_DIRECTORY,FN,CONVERT_TO_GEOTIFF,.TRUE.,IWX_BAND)
      ENDIF

      IF (DUMP_SPREAD_RATE) THEN
         FN = 'head_fire_spread_rate_' // THREE_IWX_BAND
         CALL WRITE_BIL_RASTER_THREADSAFE(SPREAD_RATE_TO_DUMP,OUTPUTS_DIRECTORY,FN,CONVERT_TO_GEOTIFF,.TRUE.,IWX_BAND)
      ENDIF
...
IF (MULTIPLE_HOSTS) THEN
   ...
   CALL MPI_BCAST(STATS_PROB                      , NUM_CASES_TOTAL, MPI_REAL   , 0, MPI_COMM_HOST_IRANK0, IERR)
...
IF (MODE .NE. 2) THEN
   ...
   IF (IRANK_WORLD .EQ. 0) THEN
      ...
      IF (DUMP_PROGRESS_MESSAGES) CALL WRITE_PROGRESS_MESSAGE(MESSAGESTR)
   ENDIF
   ...
   DO WHILE (ICASE .LT. NUM_CASES_TOTAL)
      ...
      IF (IWX_BAND .NE. IWX_BAND_LAST) THEN
         IF (IWX_BAND .LT. 1) THEN
            WRITE(*,*) 'CYCLING BECAUSE IWX_BAND .LT. 1'
            CYCLE
         ENDIF 
          
         IF (IWX_BAND + NUM_METEOROLOGY_TIMES - 1 .GT. WS%NBANDS) THEN
            WRITE(*,*) 'CYCLING BECAUSE IWX_BAND + NUM_METEOROLOGY_TIMES - 1 .GT. WS%NBANDS'
            CYCLE
         ENDIF 
...
! Start by distributing case to run to all slave nodes
      ...
      DO WHILE (TOTALCASESRUN .LT. NUM_CASES_TOTAL)
         ...
         IF (DUMP_PROGRESS_MESSAGES .AND. NUM_CASES_TOTAL .GE. 10) THEN
            IF (MODULO (TOTALCASESRUN, NUM_CASES_TOTAL/10) .EQ. 0) THEN
               WRITE(MESSAGESTR, '(A,I4,A,I4,A)') 'ELMFIRE has run ', TOTALCASESRUN, ' of ', &
                                                  NUM_CASES_TOTAL, ' ensemble members'
               CALL WRITE_PROGRESS_MESSAGE(MESSAGESTR)
            ENDIF
         ENDIF
         ...
         IF (DUMP_INTERMEDIATE_TIMINGS) CALL WRITE_TIMINGS_TO_DISK

         IF (DUMP_HOURLY_RASTERS .AND. DUMP_HOURLY_RASTERS_ASAP) THEN
            IWX_BAND_TO_DUMP = STATS_IWX_BAND_START(ICASE_RECV)
            IF (HOURLY_OUTPUTS_FROM_STARTING_WX_BAND_DUMPED(IWX_BAND_TO_DUMP)) CYCLE
            NUM_CASES_COMPLETE_PER_STARTING_WX_BAND(IWX_BAND_TO_DUMP) = NUM_CASES_COMPLETE_PER_STARTING_WX_BAND(IWX_BAND_TO_DUMP) + 1

            IF (NUM_CASES_PER_STARTING_WX_BAND(IWX_BAND_TO_DUMP) .NE. NUM_CASES_COMPLETE_PER_STARTING_WX_BAND(IWX_BAND_TO_DUMP) ) CYCLE

            HOURLY_OUTPUTS_FROM_STARTING_WX_BAND_DUMPED(IWX_BAND_TO_DUMP) = .TRUE.

            IF (DUMP_SURFACE_FIRE_AREA) THEN
               PREFIX = 'surface_fire_area'
               CALL FIRE_SIZE_STATS_TO_RASTERS_HOURLY(PREFIX, STATS_SURFACE_FIRE_AREA(:), IWX_BAND_TO_DUMP )
            ENDIF

            IF (DUMP_CROWN_FIRE_AREA) THEN
               PREFIX = 'crown_fire_area'
               CALL FIRE_SIZE_STATS_TO_RASTERS_HOURLY(PREFIX, STATS_CROWN_FIRE_AREA(:), IWX_BAND_TO_DUMP )
            ENDIF

            IF (DUMP_FIRE_VOLUME) THEN
               PREFIX = 'fire_volume'
               CALL FIRE_SIZE_STATS_TO_RASTERS_HOURLY(PREFIX, STATS_FIRE_VOLUME(:), IWX_BAND_TO_DUMP )
            ENDIF

            IF (DUMP_AFFECTED_POPULATION) THEN
               PREFIX = 'affected_population'
               CALL FIRE_SIZE_STATS_TO_RASTERS_HOURLY(PREFIX, STATS_AFFECTED_POPULATION(:), IWX_BAND_TO_DUMP )
            ENDIF

            IF (DUMP_AFFECTED_REAL_ESTATE_VALUE) THEN
               PREFIX = 'affected_real_estate_value'
               CALL FIRE_SIZE_STATS_TO_RASTERS_HOURLY(PREFIX, STATS_AFFECTED_REAL_ESTATE_VALUE(:), IWX_BAND_TO_DUMP )
            ENDIF

            IF (DUMP_AFFECTED_LAND_VALUE) THEN
               PREFIX = 'affected_land_value'
               CALL FIRE_SIZE_STATS_TO_RASTERS_HOURLY(PREFIX, STATS_AFFECTED_LAND_VALUE(:), IWX_BAND_TO_DUMP )
            ENDIF
         ...
         ENDIF !DUMP_HOURLY_RASTERS .AND. DUMP_HOURLY_RASTERS_ASAP
